...
describe("Handler", () => {
  it("Parses the event data into JSON", () => {
    ...
    it('Check cancelled orders have quantity = 0', () => {
      ...
      const orderQuantities = cancelledOrder.ORDER_LINES.map(orderLine => parseInt(orderLine.QUANTITY));
    ...
    it('Check remaining orders are fulfillments', () => {
      ...
      const orderQuantities = fulfilledOrder.ORDER_LINES.map(orderLine => parseInt(orderLine.QUANTITY));
